...
```{r}
goal_4_geom_point <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Distributions of SDG 3 & 4 Scores",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       fill = "Region")

ggplotly(goal_4_geom_point)
```
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("GnBu", goal_5_score)(goal_5_score), label = mytext)
```
