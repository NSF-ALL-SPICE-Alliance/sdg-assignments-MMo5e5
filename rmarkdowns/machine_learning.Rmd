...
```{r}
k2 <- kmeans(sdr_data_imputed, centers = 5)
```
...
```{r}
fp_matmort <- randomForest(normalized_score_sdg5_familypl ~ .,
...
```{r}
importance_df <- as.data.frame(fp_matmort$importance)
```
...
```{r}
pdp::partial(fp_matmort, pred.var = "normalized_score_sdg5_familypl", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(fp_matmort, pred.var = c("normalized_score_sdg5_familypl", "normalized_score_sdg3_u5mort"))
# Default PDP
plotPartial(pd)
```
